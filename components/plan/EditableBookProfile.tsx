...
export default function EditableBookProfile({ bookId, book, defaultCollapsed = false }: Props) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Any profile save may affect the plan — flag it for regeneration.
      // sessionStorage persists through refreshes; the custom event also
      // updates PlanView's banner immediately when on the same page.
  ...
  async function handleRegenerate() {
    ...
    try {
      ...
      sessionStorage.removeItem(`plan-needs-regen-${bookId}`)
  ...
  return (
    <div className="mb-8">
      ...
      {/* Redraft banner — appears after any profile save */}
      {profileSaved && (
        <div className="mt-3 flex items-center justify-between gap-2 bg-brand-accent/20 border border-brand-accent/40 rounded-lg px-4 py-2.5">
          <p className="text-xs text-brand-coal">Profile updated — redraft your plan to apply the changes.</p>
          <div className="flex items-center gap-3 shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-semibold text-brand-button hover:opacity-80 disabled:opacity-50 transition-opacity"
            >
              {regenerating ? (
                <>
